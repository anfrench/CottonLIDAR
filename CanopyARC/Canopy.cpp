...
void printCanopy(pclCluster cloud);
...
void printCanopy(pclCluster cloud)
{
	cout<<"Northing,Easting,Height\n";
	double x,y,z;
	for(int i=0; i<cloud.cloud->points.size(); i++)
	{
		x=cloud.cloud->points[i].x+409000;
		y=cloud.cloud->points[i].y+3659500;
		z=cloud.cloud->points[i].z-2.1;
